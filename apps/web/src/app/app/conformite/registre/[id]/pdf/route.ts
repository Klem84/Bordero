...
/**
 * Téléchargement sécurisé du PDF d'un bordereau via une URL signée à durée limitée.
 * La session vérifie d'abord (RLS) que l'utilisateur a accès à ce bordereau.
 */
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const supabase = await createClient();
  const { data } = await supabase.from('bordereaux').select('pdf_url').eq('id', id).maybeSingle();
